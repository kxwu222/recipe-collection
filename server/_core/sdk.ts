...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    try {
      ...
      console.warn("[Auth] Authentication failed, falling back to mock user in development:", String(error));
      
      const mockOpenId = "mock-openid";
      ...
      if (!user) {
        try {
          ...
          console.error("[Auth] Failed to create mock user:", dbError);
        }
      }
      
      if (user) {
        try {
          const households = await db.getHouseholdsByUserId(user.id);
          if (households.length === 0) {
            const inviteCode = "mockinvitecode123";
            const householdId = await db.createHousehold("My Shared Recipes", user.id, inviteCode);
            await db.addHouseholdMember(householdId, user.id, "admin");
            
            // Seed a sample recipe
            const recipeId = await db.createRecipe({
              householdId,
              name: "Tomato Egg / 番茄炒蛋",
              nameEn: "Tomato Scrambled Eggs",
              description: "A classic home-cooked Chinese dish that is quick and tasty.",
            ...
            await db.createCookingStep({
              recipeId,
              stepNumber: 2,
              instruction: "Stir fry the tomatoes until soft, then mix in the eggs.",
            });
          }
        } catch (dbError) {
          console.error("[Auth] Failed to create mock household/seed data:", dbError);
        }
      }
